...
# and posts one to Bluesky per run.
...
# helper: record table as posted and exit cleanly ------------------------------
record_and_exit <- function(id, title, tables, file, status = 0) {
  tables <- bind_rows(tables, tibble(id = id, title = title, posted_date = as.character(Sys.Date())))
  dir.create(dirname(file), showWarnings = FALSE, recursive = TRUE)
  write_csv(tables, file)
  quit(status = status)
}
...
if (is.null(df) || nrow(df) == 0 || !"value" %in% names(df)) {
  ...
  record_and_exit(tbl_id, tbl_title, posted_tables, posted_file)
}
...
# columns with too many levels are trimmed to fit
MAX_COLOR <- 6
MAX_FACET <- 12
group_col <- NULL
facet_col <- NULL
...
p <- tryCatch({
  ...
  if (!is.na(time_col) && !is.null(group_col)) {
    ...
    # --- Case 4: nothing useful → bail out ------------------------------------
  } else {
    NULL
  }
